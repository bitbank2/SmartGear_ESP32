...
/* Copyright 2002 BitBank Software, Inc.             */
...
#endif //
      pulDest = (uint32_t *)&pLocalBitmap[iPitch * (pGBMachine->iScanLine % 24)];
...
#endif // _WIN32_WCE
if ((pGBMachine->iScanLine % 24) == 23) // time to display the row
   SPI_LCD_DrawRow(pGBMachine->iScanLine - 23);
} /* GBDrawScanline() */
...
unsigned short us1, us2;
   ...
   for (i=0; i<256 /*32768*/; i++) // 32768 possible combinations of 15-stage counter
      {
...
//           g_print("GBC - Head2Head play enabled; display width = 320");
  ...
  if (bVerbose)
  {
   Serial.print("About to try to load the ROM: ");
   Serial.println(pszROM);
  }
...
//    pGBTemp = EMUAlloc(sizeof(GBMACHINE)); // local machine
    ...
    pGBTemp->pVRAM = (unsigned char *)EMUAlloc(0x4300);
    if (bHead2Head)
        gbmachine[1].pVRAM = (unsigned char *)EMUAlloc(0x4300);
    ...
    pGBTemp->pBankRAM = (unsigned char *)EMUAlloc(pGBTemp->iRAMSize+64); // up to 128k of banked RAM in the cartridge
    ...
    if (bHead2Head)
        {
    	gbmachine[1].pBankRAM = (unsigned char *)EMUAlloc(pGBTemp->iRAMSize+64);
        memset(gbmachine[1].pBankRAM, 0xff, pGBTemp->iRAMSize);
        }
    pGBTemp->pWorkRAM = (unsigned char *)EMUAlloc(0x8000); // work RAM at 0xd000
    if (bHead2Head)
    	gbmachine[1].pWorkRAM = (unsigned char *)EMUAlloc(0x8000);
...
// Set up the memory area pointers for save/load and networking
    ...
    pGBTemp->pEMUH = (EMUHANDLERS *)EMUAlloc(16*sizeof(EMUHANDLERS));
    ...
    if (iGBRefCount == 0) // not allocated yet
       {
       pWaveQueue = (WAVEQUEUE*)EMUAlloc(8192 * sizeof (WAVEQUEUE)); // up to 8k entries (3 bytes each) per frame
       if (pWaveQueue == NULL)
          {
          iError = SG_OUTOFMEMORY;
          goto gb_init_leave;
          }
       pNoise7 = (unsigned char *)EMUAlloc(128);
       if (pNoise7 == NULL)
          {
          iError = SG_OUTOFMEMORY;
          goto gb_init_leave;
          }
       pNoise15 = (unsigned char *)EMUAlloc(256/*32768*/);
       if (pNoise15 == NULL)
          {
          iError = SG_OUTOFMEMORY;
          goto gb_init_leave;
       ...
       ulAttrColor = (uint32_t *)EMUAlloc(256*sizeof(long));
       if (ulAttrColor == NULL)
          {
          iError = SG_OUTOFMEMORY;
          goto gb_init_leave;
