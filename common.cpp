...
static File myfile;
static UNZIP zip; // statically allocate the UNZIP structure (41K)
//
...
void * myOpen(const char *filename, int32_t *size) {
  ...
  Serial.printf("tried opening %s, result %d\n", filename, (int)(unsigned long)myfile);
  *size = myfile.size();
  return (void *)&myfile;
}
void myClose(void *p) {
...
#ifdef TEENSYDUINO
extern uint8_t pCartMem[]; // PSRAM to hold cartridge data
#endif
...
char *d, *pTemp;
    ...
    pTemp = (char *)EMUAlloc(0x800000); // largest size of any single ROM file
    i = 0;
...
/* First, try to read them from the ZIP file */
       ...
       rc = zip.openZIP(szName, myOpen, myClose, myRead, mySeek);
       if (rc != UNZ_OK)
          return TRUE; // error
       while (roms[i].szROMName) /* Load ROMs from the ZIP file directly */
          {
          rc = zip.locateFile(roms[i].szROMName);
          if (rc == UNZ_END_OF_LIST_OF_FILE) /* Report the file not found */
             {
//              __android_log_print(ANDROID_LOG_VERBOSE, "EMULoadRoms", "file not found: %s", roms[i].szROMName);
          ...
          rc = zip.openCurrentFile(); /* Try to open the file we want */
          rc = zip.readCurrentFile((uint8_t *)&mem_map[roms[i].iROMStart + MEM_ROMRAM], roms[i].iROMLen);
...
BOOL bSMD = FALSE;
   ...
   if (strcmp((char *)&szROM[i-4], ".ZIP") == 0 || strcmp((char *)&szROM[i-4], ".zip") == 0) // it's a zip file, load it another way
      {
#ifdef _UNICODE
      ...
      while (rc == UNZ_OK) // look for a *.BIN file
         ...
         i = (int)strlen(szTemp);
         if (strcmp(&szTemp[i-4], ".bin") == 0 || strcmp(&szTemp[i-4],".gen") == 0) // it's a BIN or GEN file, load it
            goto found_file;
         if (strcmp(&szTemp[i-4], ".smd") == 0 || strcmp(&szTemp[i-3], ".md") == 0) // it's a SMD file, load it
            {
            bSMD = TRUE;
            goto found_file;
            }
...
found_file:
      if (rc == UNZ_OK) // we found the file
         ...
         if (rc == UNZ_OK)
            ...
            *pBank = (unsigned char *)EMUAlloc(iLen+0x200);
...
//         MessageBox(hwndClient, TEXT("Unable to load game ROM image"),szROM,MB_ICONSTOP | MB_OK);
      ...
      i = (int)strlen((char *)szROM);
      if (strcmp((char *)&szROM[i-4], ".smd") == 0 || strcmp((char *)&szROM[i-3], ".md") == 0) // it's a SMD file
         bSMD = TRUE;
...
//      if (bSMD)
   ...
   if (bSMD && (iLen & 0x200)) // need to interlace the ROM image and move the header
      ...
      int x, iBlocks;
      pTemp = (unsigned char *)EMUAlloc(iLen);
...
//         pos += 2352; // don't copy the ISO header
         ...
         pTemp = (unsigned char *)EMUAlloc(iNumSectors * 2048);
...
int bISO = 1; // assume ISO image
   ...
   if (strcmp((char *)&szISO[i-4], ".ZIP") == 0) // it's a zip file, load it another way
      {
      ...
      while (rc == UNZ_OK) // look for a *.iso file
         ...
         rc = unzGetCurrentFileInfo (zHandle, &unzInfo, szTemp, 256, NULL, 0, NULL,  0);
         i = (int)strlen(szTemp);
         if (strcmp(&szTemp[i-4], ".iso") == 0) // it's a ISO image, load it
            break;
         if (strcmp(&szTemp[i-4], ".bin") == 0) // it's a BIN, load it with special care
         {
               bISO = 0;
               break;
         }
      ...
      else  if (strcmp((char *)&szISO[i-4], ".iso") == 0) // it's an uncompressed ISO
      {
            File iHandle;
      ...
      else if (strcmp((char *)&szISO[i-4],".bin") == 0) // it's a BIN file
      {
            File iHandle;
            iHandle = EMUOpen(szISO);
            if (iHandle != 0)
...
uint32_t ulCRC;
   ...
   if (strcmp((char *)&szROM[i-4], ".ZIP") == 0 || strcmp((char *)&szROM[i-4], ".zip") == 0) // it's a zip file, load it another way
      ...
      while (rc == UNZ_OK) // look for a *.PCE file
         ...
         rc = unzGetCurrentFileInfo (zHandle, &unzInfo, szTemp, 256, NULL, 0, NULL,  0);
         i = (int)strlen(szTemp);
         if (strcmp(&szTemp[i-4], ".pce") == 0 || strcmp(&szTemp[i-4], ".PCE") == 0) // it's a PCE ROM, load it
            break;
         rc = unzGoToNextFile(zHandle);
         }
      if (rc == UNZ_OK) // we found the file
...
BOOL bColeco = FALSE;
   ...
   *iGGMode = MODE_SMS; // assume SMS
   i = (int)strlen((char *)szROM);
   if (strcmp((char *)&szROM[i-4], ".ZIP") == 0 || strcmp(&szROM[i-4], ".zip") == 0) // it's a zip file, load it another way
      {
#ifdef _UNICODE
      wcstombs(szTemp, szROM, 256);
      zHandle = unzOpen(szTemp);
...
//      __android_log_print(ANDROID_LOG_VERBOSE, "smartgear", "file opened, handle=0x%08x", (int)zHandle);
      ...
      while (rc == UNZ_OK) // look for a *.GG file
         ...
         rc = unzGetCurrentFileInfo (zHandle, &unzInfo, szTemp, 256, NULL, 0, NULL,  0);
         i = (int)strlen(szTemp);
         if (strcmp(&szTemp[i-3], ".gg") == 0 || strcmp(&szTemp[i-3], ".GG") == 0) // it's a gamegear ROM, load it
            {
            *iGGMode = MODE_GG;
            break;
            }
         if (strcmp(&szTemp[i-4], ".sms") == 0 || strcmp(&szTemp[i-4], ".SMS") == 0) // it's a SMS ROM, load it
            break;
         if (strcmp(&szTemp[i-4], ".rom") == 0 || strcmp(&szTemp[i-4], ".ROM") == 0) // it's a Colecovision ROM, load it
            {
            bColeco = TRUE;
            break;
            }
...
//            __android_log_print(ANDROID_LOG_VERBOSE, "smartgear", "file size = %d", iLen);
            if (bColeco)
               {
               *pBank = (unsigned char *)EMUAlloc(iLen+8192);
               ...
               {
               *pBank = (unsigned char *)EMUAlloc(iLen);
...
//               __android_log_print(ANDROID_LOG_VERBOSE, "smartgear", "finished reading file");
      ...
      iHandle = EMUOpenRO((char *)szROM);
      i = (int)strlen((char *)szROM);
      if (strcmp((char *)&szROM[i-3], ".gg") == 0 || strcmp((char *)&szROM[i-3], ".GG") == 0) // it's a gamegear ROM
         *iGGMode = MODE_GG;
      if (strcmp((char *)&szROM[i-3], ".rom") == 0 || strcmp((char *)&szROM[i-3], ".ROM") == 0) // it's a Coleco ROM
         bColeco = TRUE;
      if (iHandle == 0)
         {
//         MessageBox(hwndClient, TEXT("Unable to load game ROM image"),szROM,MB_ICONSTOP | MB_OK);
...
char szTemp[256];
   ...
   if (strcmp((char *)&szROM[i-4], ".ZIP") == 0 || strcmp((char *)&szROM[i-4], ".zip") == 0) // it's a zip file, load it another way
      ...
      while (rc == UNZ_OK) // look for a *.GG file
         ...
         rc = unzGetCurrentFileInfo (zHandle, &unzInfo, szTemp, 256, NULL, 0, NULL,  0);
         i = (int)strlen(szTemp);
         if (strcmp(&szTemp[i-4], ".rom") == 0) // it's a ROM, load it
            break;
         rc = unzGoToNextFile(zHandle);
         }
      if (rc == UNZ_OK) // we found the file
...
char szTemp[256];
   ...
   if (strcmp((char *)&szROM[i-4], ".ZIP") == 0 || strcmp((char *)&szROM[i-4], ".zip") == 0) // it's a zip file, load it another way
      ...
      while (rc == UNZ_OK) // look for a *.GG file
         ...
         rc = unzGetCurrentFileInfo (zHandle, &unzInfo, szTemp, 256, NULL, 0, NULL,  0);
         i = (int)strlen(szTemp);
         if (strcmp(&szTemp[i-4], ".nes") == 0 || strcmp(&szTemp[i-4], ".NES") == 0) // it's a NES ROM, load it
            break;
         rc = unzGoToNextFile(zHandle);
         }
      if (rc == UNZ_OK) // we found the file
...
unsigned char *pMem = NULL;
   ...
   iLen = 0;
   i = (int)strlen((char *)szROM);
   if (strcmp((char *)&szROM[i-4], ".ZIP") == 0) // it's a zip file, load it another way
      {
#ifdef _UNICODE
      ...
      while (rc == UNZ_OK) // look for a *.SMC file
         ...
         rc = unzGetCurrentFileInfo (zHandle, &unzInfo, szTemp, 256, NULL, 0, NULL,  0);
         i = (int)strlen(szTemp);
         if (strcmp(&szTemp[i-4], ".smc") == 0 || strcmp(&szTemp[i-4], ".fig") == 0) // it's a SNES ROM, load it
            break;
         rc = unzGoToNextFile(zHandle);
         }
      if (rc == UNZ_OK) // we found the file
         ...
         if (rc == UNZ_OK)
            {
            iLen = (int)unzInfo.uncompressed_size;
            pMem = (unsigned char *)EMUAlloc(iLen + 0x28000); // leave room for WRAM + SRAM + PPU regs
...
//         MessageBox(hwndClient, TEXT("Unable to load game ROM image"),szROM,MB_ICONSTOP | MB_OK);
      ...
      if (iLen & 0x200) // there is a 512 byte header
         ...
         EMUSeek(iHandle, 0, 0); // start at the beginning of the file
      pMem = (unsigned char *)EMUAlloc(iLen + 0x28000);
...
#endif
      Serial.printf("EMULoadGBRoms - About to call unzOpen() on file %s\n", szTemp);
      rc = zip.openZIP(szTemp, myOpen, myClose, myRead, mySeek);
      Serial.printf("Returned from unzOpen(), rc = %d\n", rc);
      if (rc != UNZ_OK)
         {
//         MessageBox(hwndClient, TEXT("Unable to load game ROM image"),szROM,MB_ICONSTOP | MB_OK);
         return TRUE; // error opening the file
      ...
      while (rc == UNZ_OK) // look for a *.GB or *.GBC file
         {
 	Serial.printf("looking for a GB/GBC file in the zip file\n");
         rc = zip.getFileInfo (&unzInfo, szTemp, 256, NULL, 0, NULL,  0);
         i = (int)strlen(szTemp);
         if (strcmp(&szTemp[i-3], ".gb") == 0 || strcmp(&szTemp[i-3], ".GB") == 0) // it's a gameboy ROM, load it
            break;
         if (strcmp(&szTemp[i-4], ".gbc") == 0 || strcmp(&szTemp[i-4], ".GBC") == 0) // it's a gameboy color ROM, load it
            break;
         rc = zip.gotoNextFile();
         }
      if (rc == UNZ_OK) // we found the file
         {
	Serial.printf("about to call unzOpenCurrentFile\n");
         rc = zip.openCurrentFile(); /* Try to open the file we want */
	Serial.printf("returned from unzOpenCurrentFile\n");
         if (rc == UNZ_OK)
            {
            iLen = (int)unzInfo.uncompressed_size;
	Serial.printf("about to allocate %d bytes, pBank = %08x\n", iLen, (int)(unsigned long)pBank);
//            *pBank = (unsigned char *)EMUAlloc(iLen);
#ifdef TEENSYDUINO
             *pBank = pCartMem; // use PSRAM
#else
             *pBank = (unsigned char *)ps_malloc(iLen);
#endif
	Serial.printf("about to call unzReadCurrentFile\n");
            zip.readCurrentFile(*pBank, iLen); /* Read the file into memory */
            rc = zip.closeCurrentFile();
  Serial.printf("Read %d bytes of cartridge data\n", iLen);
            pTemp = *pBank;
         ...
         {
         zip.closeZIP();
         Serial.printf("Unable to load game ROM image\n");
         return TRUE; // error opening the file
         }
      zip.closeZIP();
      }
      ...
      {
#ifdef FUTURE
      unsigned char *pTemp = (unsigned char *)EMUAlloc(0x1000); // 4k buffer for reading the ROM data to flash memory
      ...
      const esp_partition_t* part = esp_partition_find_first(ESP_PARTITION_TYPE_DATA, ESP_PARTITION_SUBTYPE_DATA_SPIFFS, NULL);
         ...
         if (part == 0)
        {
          Serial.println("esp_partition_find_first() failed.\n");
          return -1;
        }
      ...
      if (iHandle == NULL)
         {
         Serial.println("Unable to load game ROM image");
         return TRUE;
         }
      iLen = EMUSeek(iHandle, 0, 2);
      EMUSeek(iHandle, 0, 0); // seek to start of ROM data
      // Prepare flash
//      err = spi_flash_erase_range(0x3f800000, (size_t) iLen);
      err = esp_partition_erase_range(part, 0, (iLen + 4095) & 0xfffff000);
      if (err != ESP_OK)
      {
         Serial.print("Error erasing flash = ");
         Serial.println((int)err);
      }
      // Read the data from SDCard and write to flash
      for (i=0; i<iLen; i += 0x1000) // read/write 4k at a time
      {
        EMURead(iHandle, pTemp, 0x1000);
 //       spi_flash_write((size_t)(0x3f800000+i), pTemp, 0x1000);
          esp_partition_write(part, i, (const void *)pTemp, 0x1000);
      }
      // Map the flash into the data (readonly) address space
      err = esp_partition_mmap(part, 0, (size_t)iLen, SPI_FLASH_MMAP_DATA, (const void **)pBank, &out_handle);
      if (err == ESP_OK)
      {
         Serial.print("ROM mapped to address: ");
         Serial.println((int)pBank);
         Serial.print("First byte = ");
         Serial.println((int)pBank[0]);
      }
      else
         Serial.println("Error mapping ROM to address");
//      *pBank = (unsigned char *)EMUAlloc(iLen);
      EMUClose(iHandle);
      free(pTemp);
#endif // FUTURE
...
Serial.println("Entering EMUTestName");

   iType = SYS_UNKNOWN;
   // See if it's a save game file
   i = (int)strlen((char *)pszName);
   if (strcmp((char *)&pszName[i-5], "SGSAV") == 0) // our save game
   ...
   if (strcmp((char *)&pszName[i-4], ".ZIP") == 0 || strcmp((char *)&pszName[i-4], ".zip") == 0) // it's a zip file, test it another way
      {
        Serial.println("It's a zip file");
// DEBUG
...
//          }
      ...
      zHandle = unzOpen(szTemp);
#else
Serial.printf("about to call unzOpen of %s\n", pszName);
      rc = zip.openZIP(pszName, myOpen, myClose, myRead, mySeek);
#endif
      if (rc != UNZ_OK) {
        Serial.printf("Opening %s failed, returning\n", pszName);
         return -1; // error opening the file
      }
      Serial.printf("about to unzGoToFirstFile of %s\n", pszName);
      zip.gotoFirstFile();
      rc = UNZ_OK;
...
//         }
      while (rc == UNZ_OK && !iType) // look for a game ROM file for the various systems
        ...
        int j;
         ...
         i = (int)strlen(szTemp);
         if (szTemp[i-4] != '.') i++;
         szExt[0] = szTemp[i-4]; // get the '.'
         for (j=1; j<4; j++)
            szExt[j] = szTemp[i-4+j] | 0x20; // force to be lower case
         szExt[4] = 0; // terminate
         Serial.printf("testing file named: %s, with extension %s\n", szTemp, szExt);
         if (strcmp(szExt, ".nes") == 0) // it's a NES ROM, try to load it
            iType = SYS_NES;
         if (strcmp(szExt, ".gg") == 0) // it's a gamegear ROM, try to load it
            iType = SYS_GAMEGEAR;
         if (strcmp(szExt, ".sms") == 0) // it's a Master System ROM, try to load it
            iType = SYS_GAMEGEAR;
         if (strcmp(szExt, ".gb") == 0) // it's a gameboy ROM, try to load it
            iType = SYS_GAMEBOY;
         if (strcmp(szExt, ".gbc") == 0) // it's a gameboy color ROM, try to load it
            iType = SYS_GAMEBOY;
         if (strcmp(szExt, ".pce") == 0) // it's a PC-Engine ROM, try to load it
            iType = SYS_TG16;
         if (strcmp(szExt, ".smd") == 0 || strcmp(szExt, ".md") == 0) // it's a Genesis ROM, try to load it
            iType = SYS_GENESIS;
         if (strcmp(&szTemp[i-4], ".bin") == 0 || strcmp(szExt,".gen") == 0) // it's a Genesis ROM, try to load it
            iType = SYS_GENESIS;
...
//            iType = 6;
      ...
      zip.closeZIP(); // we're done with the file, close it
      return iType;
      }
   else // not a zip file
      {
      ...
      i = (int)strlen((char *)pszName);
      if (pszName[i-4] != '.') i++;
      szExt[0] = pszName[i-4]; // get the '.'
      for (j=1; j<4; j++)
         szExt[j] = pszName[i-4+j] | 0x20; // force to be lower case
      szExt[4] = 0; // terminate
      if (strcmp(szExt, ".nes") == 0) // it's a NES ROM
         return SYS_NES;
      if (strcmp(szExt, ".sms") == 0) // it's a Master System ROM
         return SYS_GAMEGEAR;
      if (strcmp(szExt, ".gg ") == 0) // it's a gamegear ROM
         return SYS_GAMEGEAR;
      if (strcmp(szExt, ".gb ") == 0) // it's a gameboy ROM
         return SYS_GAMEBOY;
      if (strcmp(szExt, ".gbc") == 0) // it's a gameboy color ROM
         return SYS_GAMEBOY;
      if (strcmp(szExt, ".pce") == 0) // it's a PC-Engine ROM
         return SYS_TG16;
      if (strcmp(szExt, ".smd") == 0 || strcmp(szExt, ".md ") == 0) // it's a Genesis ROM
         return SYS_GENESIS;
      if (strcmp(szExt, ".bin") == 0 || strcmp(szExt,".gen") == 0) // it's a Genesis ROM
         return SYS_GENESIS;
...
//	   __android_log_print(ANDROID_LOG_VERBOSE, "smartgear", "File too small, len = %d", iFileLen);
   ...
   pTemp = (unsigned char *)EMUAlloc(iFileLen+2);
...
#endif
   pTemp = (unsigned char *)EMUAlloc(MAX_SAVE_SIZE);
   }
else
   {
...
int ParseNumber(char *pBuf, int *i, int iLen)
{
    ...
    while (j < iLen && pBuf[j] != '-' && (pBuf[j] < '0' || pBuf[j] > '9'))
    {
        j++;
    }
    if (j >= iLen) // went past end, problem
        return 0;
    while (j < iLen && k < 32 && (pBuf[j] == '-' || (pBuf[j] >= '0' && pBuf[j] <= '9')))
    {
        cTemp[k++] = pBuf[j++];
    }
...
int SG_ParseConfigFile(char *szConfig, char *szDir)
{
    ...
    if (fh == 0) return -1; // no config file!
    // Set default values
    iLCDType = -1;
    iDisplayType = -1;
...
//    if (pBuf == NULL) {};
    strcpy(szDir, (char *)"/"); // start in root dir - DEBUG
    ...
    while (i < iLen)
    {
        ...
        else if (memcmp(&pBuf[i], "lcd_type",8) == 0)
        {
            ...
            else if (memcmp(&pBuf[i], "st7789", 6) == 0)
            {
                iLCDType = LCD_ST7789;
                iLCDX = iLCDY = 240;
            }
            ...
            else if (memcmp(&pBuf[i], "ili9342",7) == 0)
            {
                iLCDType = LCD_ILI9342;
                iLCDX = 320; iLCDY = 240;
            }
