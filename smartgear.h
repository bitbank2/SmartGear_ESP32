...
int b16BitAudio;
int bHead2Head;
int bLoaded; // for multithreaded access
int bRewound; // game was just rewound
...
// PlayProc(GAME_BLOB *pBlob, int bVideo, int bAudio, ULONG ulKeys);
//
...
typedef void (*GAMEPLAYPROC)(GAME_BLOB *, int, int, uint32_t);
...
typedef void (*PLAYPROC)(GAME_BLOB *, int, int, uint32_t);
...
int SGSaveHighScore(char *szGame, unsigned char *pMem, int iLen);
int SGLoadHighScore(char *szGame, unsigned char *pMem, int iLen);
int SGLoadGame(char *, GAME_BLOB *pBlob, int iGame);
int SGSaveGame(char *, GAME_BLOB *pBlob, int iGame);
void SG_GetLeafName(char *fname, char *leaf);
int EMULoadGBRoms(char *szROM, unsigned char **pBank);
